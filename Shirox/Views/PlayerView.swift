...
struct PlayerView: View {
    ...
    /// KVO watching a *source/quality switch* item for a fast failure so we can revert to the
    /// stream we were watching (v2.15 behavior), separate from `statusObserver`'s refetch path.
    @State private var sourceSwitchObserver: NSKeyValueObservation? = nil
    ...
    var body: some View {
        ...
        .onDisappear {
            ...
            statusObserver = nil
            sourceSwitchObserver?.invalidate()
            sourceSwitchObserver = nil
    ...
    private func setupPlayer() {
        ...
        // (cb0badd) Plain KVO via observeItemStatus(item) — see that method: AsyncPublisher
        // dropped the fast .unknown → .failed transition expired CDN URLs produce.
    ...
    /// launch item, `swapStream` for episode advances and refetch recoveries.
    ///
    ...
    private func switchQuality(_ next: StreamResult) {
        ...
        // a dead button when both resolved to one URL. Runs first so an aliased
        // source stays a cheap metadata swap even while casting.
        ...
        // (e6491f3) While casting, quality is the receiver's business: rebuilding the local
        // item would leave the TV on the old rendition and start the new one on the phone.
        ...
        // (cb0badd) Plain KVO instead of `publisher(for:).values` — AsyncPublisher drops
        // the fast .unknown → .failed transition an expired CDN URL produces, which is
        // precisely the failure this revert path exists to catch. `.initial` also covers an
        // item that failed before we attached.
        sourceSwitchObserver?.invalidate()
        sourceSwitchObserver = newItem.observe(\.status, options: [.initial, .new]) { [weak previousItem] observedItem, _ in
            let status = observedItem.status
            DispatchQueue.main.async {
                guard observedItem === player?.currentItem else { return }
                switch status {
                case .failed:
                    Logger.shared.log("[Player] Source switch to '\(next.title)' failed (\(observedItem.error?.localizedDescription ?? "unknown")) — reverting to '\(previousStream.title)'", type: "Error")
                    if let previousItem {
    ...
    private func swapStream(_ next: StreamResult, episodeNumber: Int, allStreams: [StreamResult] = [], episodeHref: String? = nil) {
        ...
        setupPlaybackEndObserver(for: newItem)
        // (cb0badd) Every replacement item needs its own failure detection — the observer set
        // up at launch watches the item it was given, so without this a dead URL on episode 2+
        // (auto-advance, next-episode pick, sequel, refetch recovery) produced a black screen
        // with no refetch and no error. Also retire any pending source-switch revert watcher:
        // its item is no longer on screen, so its guard would no-op forever.
        sourceSwitchObserver?.invalidate()
        sourceSwitchObserver = nil
...
class PlayerHostingController<Content: View>: UIHostingController<Content> {
    ...
    override var preferredInterfaceOrientationForPresentation: UIInterfaceOrientation {
        // (cb0badd) Only steer the presentation when Force Landscape is on. UIKit consults this
        // whenever the current orientation isn't in supportedInterfaceOrientations — so returning
        // a landscape side unconditionally forced a landscape player on a user who had the feature
