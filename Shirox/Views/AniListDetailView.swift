...
struct AniListDetailView: View {
    ...
    var body: some View {
        ...
        .adaptiveSheet(item: $pendingDownloadEpisodeNumber) { item in
            ...
            // v2.22 — Unified download UI: the Download button on an
            // episode row now offers "This Episode" (the exact existing
            // module/stream flow) and "Download Range" (From/To + summary)
            // in one clean, custom sheet — the separate range button in
            // the Episodes header is gone (merged here).
            UnifiedAnimeDownloadSheet(
                mediaId: media.id,
                animeTitle: media.title.searchTitle,
                ...
                onEpisodeStreamsLoaded: { streams, episodeHref in
                ...
                onRangeConfirmed: { numbers in
                    selectedEpisodeNumbers = numbers
                    pendingDownloadEpisodeNumber = nil
